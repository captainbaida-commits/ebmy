...
CHECK_INTERVAL = int(os.getenv("CHECK_INTERVAL", "10"))  # не используется, но оставлено
...
RETRY_DELAY = 2
USD_TO_UAH = 45          # курс доллара к гривне
EXTRA_DELIVERY_COST = 160

# Добавляем параметры для США: локация (1 = US), сортировка по новизне, 240 товаров на страницу
...
def fetch_ebay_html_with_fixed_pair():
    ...
    # Куки для США
    cookies = {'ebay': '%2F', 'm': 'US', 's': 'US', 'siteid': '0'}
...
def extract_price_jsonld(card, url=None, soup=None):
    ...
    for price, curr in candidates:
        if curr == 'USD' or (curr == '' and str(price).startswith('$')):
            return f"${price}"
        # если валюта другая, не возвращаем
    return None
...
def extract_shipping(card, item_price=None, range_prices=None):
    # Сначала ищем элемент с классом .su-styled-text.secondary.large, содержащий delivery/shipping
    for elem in card.select('.su-styled-text.secondary.large'):
        ...
        if 'delivery' in text_lower or 'shipping' in text_lower:
            if 'free' in text_lower:
                return "Бесплатно"
            # Извлекаем цену, только если она в USD
    ...
    # Регулярные выражения по HTML (только с $)
...
def extract_buy_it_now_info(card):
    ...
    return True, None  # есть надпись, но цену не нашли или не USD
...
def parse_ebay_listings(html, max_items=MAX_ITEMS):
    ...
    for card in cards:
        ...
        # Если доставка не USD и не "Бесплатно" – можно оставить как текст, но для расчёта она не используется
...
def calculate_total_price(price_str, shipping_str, buy_it_now_price_str=None, is_auction=False):
    if not price_str or price_str == "Цена не указана (не USD)" or "до" in price_str:
        return None
...
if __name__ == "__main__":
    send_telegram_message("🚀 Бот для eBay США запущен. Интервал 10-22 сек, команды /stop /start")
