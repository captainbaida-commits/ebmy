...
MAX_ITEMS = 60
RETRY_DELAY = 2
USD_TO_UAH = float(os.getenv("USD_TO_UAH", "45"))
EXTRA_DELIVERY_COST = int(os.getenv("EXTRA_DELIVERY_COST", "160"))
...
def normalize_ebay_search_url(raw_url):
    """Normalize the configured search URL for the US eBay marketplace.

    The Render variable may still contain a legacy ebay.co.uk URL from the previous marketplace bot.
    We preserve the user's search query/path, but force the marketplace host to
    www.ebay.com and replace only the parameters that define location/page size/sort.

    On ebay.com, LH_PrefLoc=1 means items located in the United States.
    _ipg=60 keeps the HTML smaller and more reliable through proxies while the parser
    processes the complete first result page (MAX_ITEMS=60).
    """
    if not raw_url:
        return raw_url
    ...
    if host == 'ebay.com' or host.endswith('.ebay.com') or host == 'ebay.co.uk' or host.endswith('.ebay.co.uk') or host == 'ebay.us' or host.endswith('.ebay.us'):
        netloc = 'www.ebay.com'
        if parts.port:
            netloc += f':{parts.port}'
        scheme = 'https'
    ...
    cleaned.extend([
        ('LH_PrefLoc', '1'),   # US Only on ebay.com
        ('_ipg', '60'),        # smaller/stabler response through proxy
...
# ============ ПРОФИЛИ БРАУЗЕРОВ ============
# По свежему US-логу именно Chrome150 дал HTTP 200 на IP, где Firefox147
...
def init_db():
    with get_db_connection('ebay_us_bot_init') as conn:
...
def get_bot_state(key, default=None):
    with get_db_connection('ebay_us_bot_state_read') as conn:
        with conn.cursor() as cur:
...
def set_bot_state(key, value):
    with get_db_connection('ebay_us_bot_state_write') as conn:
        with conn.cursor() as cur:
            cur.execute(
                """
...
def initialize_seen_count_cache():
    ...
    try:
        with get_db_connection('ebay_us_seen_count_init') as conn:
            with conn.cursor() as cur:
...
def claim_new_seen_ids(item_ids):
    ...
    if not unique_ids:
        return set()
    with get_db_connection('ebay_us_seen_claim') as conn:
        with conn.cursor() as cur:
...
def is_db_empty():
    ...
    if cached is not None:
        return cached == 0
    with get_db_connection('ebay_us_db_empty') as conn:
        with conn.cursor() as cur:
...
def get_exact_auction_by_id(item_id):
    with get_db_connection('ebay_us_auction_exact_get') as conn:
        with conn.cursor() as cur:
            cur.execute(
...
def get_saved_auction_state(item_id):
    ...
    if not item_id:
        return None
    with get_db_connection('ebay_us_auction_saved_state') as conn:
        with conn.cursor() as cur:
...
def enqueue_auction_link(url):
    ...
    canonical = f"https://www.ebay.com/itm/{item_id}" if item_id else str(url or '')
    key = _auction_queue_key(canonical, item_id)
    duplicate_state = None
    existing_message_id = None
    is_new = False

    with get_db_connection('ebay_us_auction_link_enqueue') as conn:
        with conn.cursor() as cur:
            if item_id:
...
def set_auction_link_status_message(queue_key, message_id):
    if not queue_key or not message_id:
        return False
    with get_db_connection('ebay_us_auction_link_message') as conn:
        with conn.cursor() as cur:
            cur.execute(
                """
...
def get_next_auction_link_job_state():
    ...
    """
    with get_db_connection('ebay_us_auction_link_due') as conn:
        with conn.cursor() as cur:
            cur.execute(
                """
...
def list_queued_auction_links(limit=20):
    with get_db_connection('ebay_us_auction_link_list') as conn:
        with conn.cursor() as cur:
            cur.execute(
                """
...
def postpone_auction_link_job(queue_key, attempt_count, reason='temporary_unavailable'):
    ...
    with get_db_connection('ebay_us_auction_link_retry') as conn:
        with conn.cursor() as cur:
            cur.execute(
                """
                UPDATE auction_link_queue
...
def delete_auction_link_job(queue_key):
    with get_db_connection('ebay_us_auction_link_delete') as conn:
        with conn.cursor() as cur:
...
def get_auction_durable_counts():
    ...
    with get_db_connection('ebay_us_auction_durable_counts') as conn:
        with conn.cursor() as cur:
            cur.execute(
                """
                SELECT
...
def _advance_queued_auctions_for_new_fixed(limit=10):
    ...
    try:
        with get_db_connection('ebay_us_auction_queue_new_fixed') as conn:
            with conn.cursor() as cur:
                cur.execute(
                    """
                    WITH picked AS (
...
def transfer_auction_link_status_to_pending(item_id):
    ...
    if not item_id:
        return False
    with get_db_connection('ebay_us_auction_link_message_transfer') as conn:
        with conn.cursor() as cur:
            cur.execute(
                """
...
def get_auction_link_status_message(item_id):
    if not item_id:
        return None
    with get_db_connection('ebay_us_auction_link_message_get') as conn:
        with conn.cursor() as cur:
            cur.execute(
...
def get_pending_status_message(item_id):
    with get_db_connection('ebay_us_auction_pending_message_get') as conn:
        with conn.cursor() as cur:
...
def set_pending_status_message(item_id, message_id):
    if not item_id or not message_id:
        return False
    with get_db_connection('ebay_us_auction_pending_message_set') as conn:
        with conn.cursor() as cur:
            cur.execute(
...
def save_pending_auction(
    ...
    canonical_url = f"https://www.ebay.com/itm/{item_id}"
...
def list_pending_auctions(limit=20):
    with get_db_connection('ebay_us_auction_pending_list') as conn:
        with conn.cursor() as cur:
            cur.execute(
                """
...
def get_due_pending_auctions(limit=1):
    with get_db_connection('ebay_us_auction_pending_due') as conn:
        with conn.cursor() as cur:
            cur.execute(
                """
                SELECT item_id, url, title, end_earliest_utc, end_latest_utc,
...
def postpone_pending_auction(item_id, delay_seconds=None):
    ...
    with get_db_connection('ebay_us_auction_pending_retry') as conn:
        with conn.cursor() as cur:
            cur.execute(
                """
                UPDATE auction_pending
...
def delete_pending_auction(item_id, wake=True):
    with get_db_connection('ebay_us_auction_pending_delete') as conn:
        with conn.cursor() as cur:
...
def delete_auction_any(item_id):
    ...
    with get_db_connection('ebay_us_auction_delete_any') as conn:
        with conn.cursor() as cur:
            cur.execute("DELETE FROM auction_reminders WHERE item_id=%s RETURNING item_id", (item_id,))
...
def delete_all_auctions():
    ...
    status_message_ids = []
    with get_db_connection('ebay_us_auction_delete_all') as conn:
        with conn.cursor() as cur:
        ...
        try:
            if not is_paused:
                ...
                if elapsed >= CONNECTION_ALERT_AFTER and not already_sent:
                    ...
                    if had_success:
                        status_line = f"⏱ Уже <b>{minutes} мин.</b> нет успешного подключения к eBay США."
                    else:
                        status_line = f"⏱ Уже <b>{minutes} мин.</b> после запуска нет успешного подключения к eBay США."

                    message = (
                        "⚠️ <b>eBay США — проверьте подключение</b> 🇺🇸\n\n"
                        f"{status_line}\n"
    ...
    for variant in _ebay_decoded_variants(raw_html):
        ...
        for source in sources:
            ...
            for km in key_re.finditer(source):
                ...
                # V6.12 его намеренно НЕ принимает как фиксированный market timezone: реальный ux-timer может
...
def _visible_tzinfo(tz_name):
    if not tz_name:
        return None
    hours = _VISIBLE_TZ_OFFSETS.get(str(tz_name).upper())
    return timezone(timedelta(hours=hours)) if hours is not None else None


def _parse_visible_market_datetime(text, observed_at=None, default_tz=None):
    """Parse an absolute marketplace date/time to UTC with minute precision.

    US eBay can expose PDT/PST/MDT/MST/CDT/CST/EDT/EST, while a proxy/browser can
    still localize the visible clock differently. Therefore a time WITHOUT an explicit
    timezone is never assumed to be New York (or any other US zone). It is accepted only
    when the caller explicitly supplies ``default_tz``.
    """
    if not text:
        return None
    ...
    tz_group = r'(BST|GMT|UTC|PDT|PST|MDT|MST|CDT|CST|EDT|EST)'

    # Today / Tomorrow at 7:35 PM EDT
    rel = re.search(
        rf'\b(Today|Tomorrow)\b(?:\s+at)?\s+(\d{{1,2}}):(\d{{2}})(?::(\d{{2}}))?\s*(AM|PM)?\s*{tz_group}?\b',
        raw, re.I,
    )
    if rel:
        word, hh, mm, ss, ampm, tz_name = rel.groups()
        explicit_tz = _visible_tzinfo(tz_name)
        context_tz = explicit_tz or default_tz
        if context_tz is None:
            return None
        ...
        try:
            local_dt = datetime(day.year, day.month, day.day, hh, mm, ss, tzinfo=context_tz)
        except ValueError:
            return None
        return local_dt.astimezone(timezone.utc)

    # Day-first or US month-first absolute date. Both accept all explicit TZ abbreviations.
    day_first = re.search(
        rf'(?:Mon|Tue|Wed|Thu|Fri|Sat|Sun)?[,]?\s*'
        rf'(\d{{1,2}})\s+([A-Za-z]{{3,9}})(?:[,]?\s+(20\d{{2}}))?[,]?\s*(?:at\s*)?'
        rf'(\d{{1,2}}):(\d{{2}})(?::(\d{{2}}))?\s*(AM|PM)?\s*{tz_group}?\b',
        raw, re.I,
    )
    month_first = re.search(
        rf'(?:Mon|Tue|Wed|Thu|Fri|Sat|Sun)?[,]?\s*'
        rf'([A-Za-z]{{3,9}})\s+(\d{{1,2}})(?:[,]?\s+(20\d{{2}}))?[,]?\s*(?:at\s*)?'
        rf'(\d{{1,2}}):(\d{{2}})(?::(\d{{2}}))?\s*(AM|PM)?\s*{tz_group}?\b',
        raw, re.I,
    )
    ...
    try:
        return datetime(year, month, day, hh, mm, ss, tzinfo=context_tz).astimezone(timezone.utc)
...
def _parse_relative_day_clock(text):
    ...
    страницы/proxy, поэтому его нельзя считать market/Kyiv напрямую. Оно используется
...
def _localized_clock_utc_candidates(relative_text, clock_text, observed_at=None):
    ...
    страницей/proxy, поэтому ни один market timezone, ни Kyiv не предполагаются. UTC определяется
...
def _extract_localized_timer_observations(raw_html, observed_at=None):
    ...
    Сам clock может быть локализован proxy/браузером — market timezone не предполагаем.
    """
    if not raw_html:
        return []
    soup = BeautifulSoup(raw_html, 'html.parser')
...
def _extract_visible_exact_market_end(visible):
    """Parse an explicit marketplace end time only when seconds + timezone are shown."""
    if not visible:
        return None, None
    label_re = re.compile(r'\b(Ends|Ended|Ending|End\s*time)(?:\s+on)?\s*:?[,\s]*', re.I)
    tz = r'(?:BST|GMT|UTC|PDT|PST|MDT|MST|CDT|CST|EDT|EST)'
    human_patterns = (
        re.compile(
            rf'(?:Mon|Tue|Wed|Thu|Fri|Sat|Sun)?[,]?\s*'
            rf'\d{{1,2}}\s+[A-Za-z]{{3,9}}[,]?\s+20\d{{2}}[,]?\s+'
            rf'\d{{1,2}}:\d{{2}}:\d{{2}}(?:\s*(?:AM|PM))?\s*{tz}', re.I,
        ),
        re.compile(
            rf'(?:Mon|Tue|Wed|Thu|Fri|Sat|Sun)?[,]?\s*'
            rf'[A-Za-z]{{3,9}}\s+\d{{1,2}}[,]?\s+20\d{{2}}[,]?\s+'
            rf'\d{{1,2}}:\d{{2}}:\d{{2}}(?:\s*(?:AM|PM))?\s*{tz}', re.I,
        ),
    )
    for lm in label_re.finditer(visible):
        frag = visible[lm.end():lm.end()+190]
        for human_pat in human_patterns:
...
def _clean_auction_title(title, item_id=None):
    ...
    clean = re.sub(r'\s*\|\s*eBay(?:\s+(?:UK|US))?\s*$', '', clean, flags=re.I).strip()
...
def parse_auction_page(html, final_url):
    ...
    if not title:
        ...
        title = re.sub(r'\s*\|\s*eBay(?:\s+(?:UK|US))?\s*$', '', title, flags=re.I).strip()
    title = _clean_auction_title(title, item_id)

    visible_exact_utc, visible_kind = _extract_visible_exact_market_end(visible)
    ...
    if structured_end_utc:
        ...
        if visible_exact_utc:
            source = f'visible_exact_market_time+{structured_source}'
        ...
        source = 'visible_exact_market_time'
    elif visible_minute_utc:
        ...
        # Clock может быть локализован по клиенту/proxy, поэтому market timezone не предполагаем.
...
def _cleanup_stale_pending_auctions():
    ...
    with get_db_connection('ebay_us_auction_pending_cleanup') as conn:
        with conn.cursor() as cur:
            cur.execute(
                """
    ...
    while True:
        ...
        try:
            if conn is None or conn.closed:
                conn = get_db_connection('ebay_us_auction_scheduler')
            ...
            for row in rows:
                ...
                current_url = str(current_url or f"https://www.ebay.com/itm/{item_id}")
                ...
                msg = (
                    f"{urgent}⏰ <b>Напоминание об аукционе США</b> 🇺🇸\n\n"
...
def _create_session(proxy, profile, timeout=None):
    # Persistent Session keeps cookies and the already-proven TCP/TLS path.
    # US marketplace identity is enforced independently of proxy geography.
    if timeout is None:
...
def extract_range_price(card):
    ...
    for i in range(len(price_spans) - 2):
        ...
        first_usd = _extract_usd_amount(first)
        third_usd = _extract_usd_amount(third)
        if 'to' in middle and first_usd and third_usd:
            return f"{first_usd} до {third_usd}"
        ...
        if parent:
            ...
            for sibling in parent.previous_siblings:
                if hasattr(sibling, 'get_text'):
                    prev_price = _extract_usd_amount(sibling.get_text(' ', strip=True))
                    if prev_price:
                        break
            for sibling in parent.next_siblings:
                if hasattr(sibling, 'get_text'):
                    next_price = _extract_usd_amount(sibling.get_text(' ', strip=True))
                    if next_price:
                        break
        ...
        try:
            ...
            if isinstance(data, dict):
                ...
                if isinstance(offers, dict):
                    ...
                    if price and str(price) != '0' and currency == 'USD':
                    ...
                    for off in offers:
                        if not isinstance(off, dict):
                            continue
                        price = off.get('price')
                        currency = str(off.get('priceCurrency', '') or '').upper()
                        if price and str(price) != '0' and currency == 'USD':
                            candidates.append(price)
        except Exception:
            pass
...
def extract_shipping(card, item_price=None, range_prices=None):
    def acceptable_amount(text):
        amount = _extract_usd_amount(text)
        if not amount:
            return None
        if range_prices and any(amount == p for p in range_prices):
            return None
        if item_price and amount == item_price:
            return None
        return amount
    ...
    for elem in card.select('.su-styled-text.secondary.large'):
        ...
        if 'delivery' in text_lower or 'shipping' in text_lower:
            if 'free' in text_lower:
                return "Бесплатно"
            amount = acceptable_amount(text)
            if amount:
                return amount
            # Keep non-price logistics text for display, but total calculation will reject it.
            if len(text) > 3:
                return text
        ...
        try:
            ...
            if isinstance(data, dict):
                ...
                for offer in offer_list:
                    ...
                    if isinstance(shipping, (int, float)) and currency == 'USD':
                        amount = f"${shipping}"
                        if range_prices and amount in range_prices:
                            continue
                        if item_price and amount == item_price:
                            continue
                        return amount
    ...
    # Plain-text/HTML fallbacks: only US-dollar amounts are accepted as costs.
    visible = html_lib.unescape(str(card))
    for m in re.finditer(r'([^<>]{0,80}(?:delivery|shipping)[^<>]{0,80})', visible, re.I):
        amount = acceptable_amount(m.group(1))
        if amount:
            return amount

    text_lower = re.sub(r'\s+', ' ', card.get_text(' ', strip=True)).lower()
    match = re.search(r'(delivery in\s+\d+[-\s]*\d*\s*(days?|weeks?|business days?|working days?))', text_lower)
    if match:
        return match.group(1).strip()
    match = re.search(r'(delivery time\s*:\s*[\w\s\d-]+)', text_lower)
    if match:
        return match.group(1).strip()
    match = re.search(r'(shipping in\s+\d+[-\s]*\d*\s*(days?|weeks?))', text_lower)
    if match:
        return match.group(1).strip()
    return None
...
def extract_buy_it_now_info(card):
    ...
    if len(price_spans) >= 2:
        amount = _extract_usd_amount(price_spans[1].get_text(' ', strip=True))
        if amount:
            return True, amount
    ...
    if parent:
        ...
        if price_elem:
            amount = _extract_usd_amount(price_elem.get_text(' ', strip=True))
            if amount:
                return True, amount
    return True, None
...
def calculate_total_price(price_str, shipping_str, buy_it_now_price_str=None, is_auction=False):
    ...
    if not price_amount:
        return None
    match = re.search(r'([\d,]+(?:\.\d{1,2})?)', price_amount)
    if not match:
        return None
    price_num = float(match.group(1).replace(',', ''))

    if shipping_str == "Бесплатно":
        shipping_num = 0.0
    elif not shipping_str or shipping_str == "не указана":
        return None
    else:
        shipping_amount = _extract_usd_amount(shipping_str)
        if not shipping_amount:
            return None
        match = re.search(r'([\d,]+(?:\.\d{1,2})?)', shipping_amount)
        if not match:
            return None
        shipping_num = float(match.group(1).replace(',', ''))
